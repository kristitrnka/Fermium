...
import static org.taumc.celeritas.lwjgl.lwjgl2.memory.Pointer.BITS32;
import static org.taumc.celeritas.lwjgl.lwjgl2.memory.Pointer.BITS64;
import static org.taumc.celeritas.lwjgl.lwjgl2.memory.Pointer.CLONG_SIZE;
import static org.taumc.celeritas.lwjgl.lwjgl2.memory.Pointer.POINTER_SHIFT;
...
public final class MemoryUtilities {
    ...
    public static void memSet(ShortBuffer ptr, int value) {
        memSet(memAddress(ptr), value, (ptr.remaining() & 0xFFFF_FFFFL) << 1);
    }
    ...
    public static void memSet(IntBuffer ptr, int value) {
        memSet(memAddress(ptr), value, (ptr.remaining() & 0xFFFF_FFFFL) << 2);
    }
    ...
    public static void memSet(LongBuffer ptr, int value) {
        memSet(memAddress(ptr), value, (ptr.remaining() & 0xFFFF_FFFFL) << 3);
    }
    ...
    public static void memCopy(ByteBuffer src, ByteBuffer dst) {
        UNSAFE.copyMemory(null, memAddress(src), null, memAddress(dst), src.remaining());
    }
    ...
    public static void memCopy(ShortBuffer src, ShortBuffer dst) {
        UNSAFE.copyMemory(null, memAddress(src), null, memAddress(dst), (src.remaining() & 0xFFFF_FFFFL) << 1);
    }
    ...
    public static void memCopy(IntBuffer src, IntBuffer dst) {
        UNSAFE.copyMemory(null, memAddress(src), null, memAddress(dst), (src.remaining() & 0xFFFF_FFFFL) << 2);
    }
    ...
    public static void memCopy(LongBuffer src, LongBuffer dst) {
        UNSAFE.copyMemory(null, memAddress(src), null, memAddress(dst), (src.remaining() & 0xFFFF_FFFFL) << 3);
    }
    ...
    public static @Nullable String memUTF8Safe(long address) {
        return address == NULL ? null
                : decodeUTF8(address, memLengthNT1(address, Integer.MAX_VALUE));
    }
    ...
    private static ByteBuffer newDirectByteBuffer(long address, int capacity) {
        try {
            @SuppressWarnings("unchecked")
