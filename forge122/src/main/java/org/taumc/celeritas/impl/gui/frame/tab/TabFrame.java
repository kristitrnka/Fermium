...
public class TabFrame extends AbstractFrame {
    ...
    private final AtomicReference<ITextComponent> tabSectionSelectedTab;
    private final AtomicReference<Integer> tabSectionScrollBarOffset;
    private Tab<?> selectedTab;
    private AbstractFrame selectedFrame;
    private final Dim2i tabSectionInner;
    private ScrollableFrame sidebarFrame;

    public TabFrame(Dim2i dim, boolean renderOutline, Multimap<String, Tab<?>> tabs, Runnable onSetTab, AtomicReference<ITextComponent> tabSectionSelectedTab, AtomicReference<Integer> tabSectionScrollBarOffset) {
        ...
        if (this.tabSectionSelectedTab.get() != null) {
            this.selectedTab = this.tabs.values().stream().filter(tab -> tab.title().getFormattedText().equals(this.tabSectionSelectedTab.get().getFormattedText())).findAny().orElse(null);
        }
    ...
    public static class Builder {
        ...
        private AtomicReference<ITextComponent> tabSectionSelectedTab = new AtomicReference<>(null);
        ...
        public Builder setTabSectionSelectedTab(AtomicReference<ITextComponent> tabSectionSelectedTab) {
            this.tabSectionSelectedTab = tabSectionSelectedTab;
            return this;
        }
