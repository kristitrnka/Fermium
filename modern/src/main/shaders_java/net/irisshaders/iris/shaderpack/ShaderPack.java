...
public class ShaderPack {
	...
	public ShaderPack(Path root, Map<String, String> changedConfigs, ImmutableList<StringPair> environmentDefines) throws IOException, UnsupportedFeatureFlagException, IllegalStateException {
		...
		if (!optionalFeatureFlags.isEmpty()) {
			optionalFeatureFlags.forEach(flag -> CeleritasShaders.logger().warn("Found flag " + flag));
		...
		CeleritasShaders.logger().info(this.profileInfo);
		...
		customNoiseTexture = shaderProperties.getNoiseTexturePath().map(path -> {
			try {
				...
				CeleritasShaders.logger().error("Unable to read the custom noise texture at " + path, e);
		...
		shaderProperties.getCustomTextures().forEach((textureStage, customTexturePropertiesMap) -> {
			...
			customTexturePropertiesMap.forEach((samplerName, path) -> {
				try {
					...
					CeleritasShaders.logger().error("Unable to read the custom texture at " + path, e);
				}
			});
		...
		shaderProperties.getIrisCustomTextures().forEach((name, texture) -> {
			try {
				...
				CeleritasShaders.logger().error("Unable to read the custom texture at " + texture.getName(), e);
			}
		});
	}
	...
	private static Optional<Properties> loadProperties(Path shaderPath, String name,
		...
		try {
			...
			CeleritasShaders.logger().error("Error loading " + name + " at " + shaderPath, e);
	...
	private static String readProperties(Path shaderPath, String name) {
		try {
			...
			CeleritasShaders.logger().debug("An " + name + " file was not found in the current shaderpack");

			return null;
		} catch (IOException e) {
			CeleritasShaders.logger().error("An IOException occurred reading " + name + " from the current shaderpack", e);
	...
	public CustomTextureData readTexture(Path root, TextureDefinition definition) throws IOException {
		...
		if (path.contains(":")) {
			...
			if (parts.length > 2) {
				CeleritasShaders.logger().warn("Resource location " + path + " contained more than two parts?");
			}
			...
			if (Files.exists(mcMetaResolvedPath)) {
				try {
					...
					CeleritasShaders.logger().error("Unable to read the custom texture mcmeta at " + mcMetaPath + ", ignoring: " + e);
				}
			}
		...
		overrides = this.overrides.computeIfAbsent(dimension, dim -> {
			if (dimensionMap.containsKey(dimension)) {
				...
				if (dimensionIds.contains(name)) {
					...
					CeleritasShaders.logger().error("Attempted to load dimension folder " + name + " for dimension " + dimension + ", but it does not exist!");
