...
public class IdMap {
	...
	private static Object2IntMap<NamespacedId> parseIdMap(Properties properties, String keyPrefix, String fileName) {
		...
		properties.forEach((keyObject, valueObject) -> {
			...
			try {
				...
				CeleritasShaders.logger().warn("Failed to parse line in " + fileName + ": invalid key " + key);
				return;
			}
			...
			for (String part : value.split("\\s+")) {
				if (part.contains("=")) {
					...
					CeleritasShaders.logger().warn("Failed to parse an ResourceLocation in " + fileName + " for the key " + key + ": state properties are currently not supported: " + part);
					continue;
				}
		...
		properties.forEach((keyObject, valueObject) -> {
			...
			for (String part : value.split("\\s+")) {
				...
				try {
					...
					CeleritasShaders.logger().warn("Unexpected error while parsing an entry from " + fileName + " for the key " + key + ":", e);
				}
			}
	...
	private static Map<NamespacedId, BlockRenderType> parseRenderTypeMap(Properties properties, String keyPrefix, String fileName) {
		...
		properties.forEach((keyObject, valueObject) -> {
			...
			if (renderType == null) {
				CeleritasShaders.logger().warn("Failed to parse line in " + fileName + ": invalid block render type: " + key);
				return;
			}
