...
public class HandRenderer {
    ...
    public boolean isHandTranslucent(InteractionHand hand) {
        ...
        // TODO: Figure out getBlockTypeIds
...
//        }
    ...
    public void renderSolid(float tickDelta, Camera camera, RenderGlobal gameRenderer, WorldRenderingPipeline pipeline) {
        if (!canRender(camera, gameRenderer) || !CeleritasShadersApi.getInstance().isShaderPackInUse()) {
            return;
        }
        Minecraft mc = Minecraft.getMinecraft();
    ...
    public void renderTranslucent(float tickDelta, Camera camera, RenderGlobal gameRenderer, WorldRenderingPipeline pipeline) {
        if (!canRender(camera, gameRenderer) || !isAnyHandTranslucent() || !CeleritasShadersApi.getInstance().isShaderPackInUse()) {
            return;
        }
        Minecraft mc = Minecraft.getMinecraft();
    ...
    private void resetProjectionMatrix() {
        GL11.glMatrixMode(GL11.GL_PROJECTION);
        GL11.glLoadIdentity();
        GL11.glMultMatrixf(MatrixRenderingState.INSTANCE.getProjectionBuffer());
        GL11.glMatrixMode(GL11.GL_MODELVIEW);
    }
